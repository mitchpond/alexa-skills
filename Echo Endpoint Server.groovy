...
mappings {
	path("/getSwitches") 				{action: [GET: "listSwitches"]}
    path("/getSwitches/:device")		{action: [GET: "getSwitchState"]}
    path("/switch/:device/:command")	{action: [GET: "setSwitchState"]}
    path("/getContact") 				{action: [GET: "listContactSensors"]}
    path("/getContact/:device") 		{action: [GET: "getContactState"]}
...
def setSwitchState() {
	def device = (switches.find({params.device.toLowerCase() == it.label.toLowerCase()})) ? it : (dimmers.find({params.device.toLowerCase() == it.label.toLowerCase()}))
    if (device) {
